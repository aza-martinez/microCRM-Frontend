...
const Producto = ({ producto }) => {
  ...
  const [eliminarProducto] = useMutation(ELIMINAR_PRODUCTO,{
      update(cache) {
          ...
          cache.writeQuery({ 
              query: OBTENER_PRODUCTOS,
              data: { obtenerProductos:  obtenerProductos.filter(productoActual => productoActual.id !== id)}
           })
      ...
      if (result.value) {
        try {
          ...
          Swal.fire("Correcto", data.eliminarProducto, "success");
            ...
            Swal.fire("Incorrecto",'No se ha podido eliminar el producto', "warning");
        }
      }
    });
  };
